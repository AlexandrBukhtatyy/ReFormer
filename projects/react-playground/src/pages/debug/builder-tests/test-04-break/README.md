...
Скопируйте папку в `src/pages/demo/test04-break/` и зарегистрируйте форму:
...
//   <FormOutlet id="test04-break" />

// ── Способ 2: как обычную страницу, если реестр не используется.
import Test04BreakPage from './pages/demo/test04-break';

// пункт в списке примеров:
{ id: 'test04-break', path: '/examples/test04-break', title: 'Test04 break', description: '' },

// маршрут внутри <Routes>:
<Route path="/examples/test04-break" element={<Test04BreakPage />} />
```
...
переложите схему в `form.schema.ts` литералом `defineJsonSchema<Test04BreakForm>({ ... })`;
