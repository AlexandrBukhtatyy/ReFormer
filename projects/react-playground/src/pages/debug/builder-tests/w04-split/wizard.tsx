// @reformer-generated 64489e9882ce
...
export interface WizardProps {
  ...
  onSubmit?: (values: W04SplitForm) => void | Promise<void>;
}
...
export function Wizard({ form, steps = [], className, config, onSubmit }: WizardProps): ReactNode {
  const wizardSteps: FormWizardStep<W04SplitForm, RenderNode<W04SplitForm>>[] = steps.map(
    (node, i) => ({
      number: i + 1,
  ...
  return (
    <FormWizard<W04SplitForm, RenderNode<W04SplitForm>>
      form={form as FormProxy<W04SplitForm>}
      className={className}
      steps={wizardSteps}
      config={config ?? {}}
      renderStepBody={(body, wizardForm) => <RenderNodeComponent node={body} form={wizardForm} />}
      onSubmit={onSubmit ? () => onSubmit(form?.getValue() as W04SplitForm) : undefined}
    />
  );
}
