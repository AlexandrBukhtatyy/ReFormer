/**
 * Schema, behavior, validation for MCP Credit Application Renderer v10 (target=renderer-react)
 *
 * Iter-10 patch verification (FormSchema layer):
 * - Patch H — `readOnly: true` (camelCase) on computed fields, `maxLength` (camelCase) on Textarea.
 * - Patch I — `computeFrom([path.personalData], ...)` subscribes to **group node**; no `as never`.
 * - D1 — Every Select / RadioGroup carries `options` in createForm componentProps.
 * - D3 — Item factories return PLAIN primitive values (NOT FieldConfig).
 * - Cycle prevention — every `watchField` has `{ immediate: false }` and length-equality guard.
 *
...
// ============================================================================
// Item factories (D3 — plain primitives, NOT FieldConfig)
// ============================================================================
...
const formSchema: FormSchema<CreditApplicationForm> = {
  ...
  initialPayment: {
    ...
    componentProps: {
      ...
      min: 0,
      step: 10000,
      readOnly: true, // Patch H — camelCase (computed)
    },
  },
  carBrand: {
    value: '',
  ...
  watchField(
    ...
    (hasProperty, ctx) => {
      if (!hasProperty) {
        ...
        if (Array.isArray(cur) && cur.length > 0) ctx.form.properties.clear();
      }
    },
    { immediate: false }
  );
  ...
  watchField(
    ...
    (hasLoans, ctx) => {
      if (!hasLoans) {
        ...
        if (Array.isArray(cur) && cur.length > 0) ctx.form.existingLoans.clear();
      }
    },
    { immediate: false }
  );
  ...
  watchField(
    ...
    (hasCoBorrower, ctx) => {
      if (!hasCoBorrower) {
        ...
        if (Array.isArray(cur) && cur.length > 0) ctx.form.coBorrowers.clear();
      }
    },
    { immediate: false }
  );
