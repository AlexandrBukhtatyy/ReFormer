...
test(`mcp-credit-${TARGET}-v${N} — walkthrough`, async ({ page }) => {
  ...
  // Чекбоксы (Radix); label-driven check
  ...
  // Best-effort assertion: at minimum the page should still be loaded.
  await expect(page).toHaveURL(/mcp-credit-application-core-v12/);
});
