...
Представления — один контракт значения, но **отдельные компоненты** (пропа `variant` нет):
`FileUploadBase` (кнопка, registry `FileUpload`), `FileUploadDropzone` (зона перетаскивания,
registry `FileUploadDropzone`), `FileUploadInput` (поле с кнопкой, registry `FileUploadInput`).
Single-изображение с превью — `FileUploadAvatar` (значение `File | RemoteFileRef | null`,
registry-имя `FileUploadAvatar`). Каждый кладётся в `component` поля как есть.
