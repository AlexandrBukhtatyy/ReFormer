...
export const radioGroupDocConfig: ComponentDocConfig = {
  ...
  variants: [
    {
      id: 'vertical',
      title: 'Вертикально (пусто)',
      description: 'Раскладка по умолчанию: ничего не выбрано.',
  ...
  componentProps: { label: 'Тип кредита', options: LOAN },
    ...
    },
    {
      ...
      description: 'Заполненное состояние — один из вариантов отмечен.',
      render: makeFieldVariant({
        initial: 'mortgage',
        component: RadioGroup,
        componentProps: { label: 'Тип кредита', options: LOAN },
      }),
      code: `const model = createModel({ loanType: 'mortgage' });`,
    },
    {
      id: 'horizontal',
      title: 'Горизонтально',
      description: 'Ориентация в ряд: className="!flex-row gap-6".',
      render: makeFieldVariant({
        initial: null,
        component: RadioGroup,
    ...
    {
      id: 'binary',
      title: 'Бинарный выбор',
      description: 'Минимальный набор из двух опций — типичный да/нет переключатель.',
      render: makeFieldVariant({
        initial: null,
        component: RadioGroup,
  ...
  options: [
    ...
    },
    {
      id: 'disabled',
      title: 'Заблокирован',
      description: 'Все варианты недоступны — через control.disable().',
      render: makeFieldVariant({
        initial: 'consumer',
        component: RadioGroup,
        componentProps: { label: 'Тип кредита', options: LOAN },
    ...
    {
      id: 'invalid',
      title: 'Невалидный (ошибка)',
      description: 'Состояние ошибки: группа помечена невалидной, значение не выбрано.',
      render: makeFieldVariant({
        initial: null,
        component: RadioGroup,
        componentProps: { label: 'Тип кредита', options: LOAN },
      ...
      code: `// touched-поле с проваленным required показывает ошибку
{
  value: model.$.loanType,
  component: RadioGroup,
  componentProps: { label: 'Тип кредита', options: LOAN },
