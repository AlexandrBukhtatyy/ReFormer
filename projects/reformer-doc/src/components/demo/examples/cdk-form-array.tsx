...
// ─── Variants (витрина состояний собранного массива) ───────────────────────

function FormArrayFilled() {
  const { form } = usePhoneForm();
  return <BasicArrayView control={form.phones} />;
}

function FormArrayEmptyState() {
  const { form } = usePhoneForm([]);
  return <BasicArrayView control={form.phones} />;
}

function FormArrayDisabled() {
  const { form } = usePhoneForm(ONE_PHONE, (f) => f.phones.disable());
  return <BasicArrayView control={form.phones} />;
}

function FormArrayInvalid() {
  const { form } = usePhoneForm(ONE_PHONE, (f) =>
    f.phones.setErrors([{ code: 'minItems', message: 'Добавьте хотя бы один телефон' }])
  );
  return <BasicArrayView control={form.phones} showError />;
}
...
export const formArrayDocConfig: ComponentDocConfig = {
  ...
  description:
    'Headless compound-компонент для динамических массивов. Почти вся поверхность — это ВОЗМОЖНОСТИ ' +
    '(ручная сборка, render-props, hook, imperative ref, asChild, кастомный render), поэтому основной ' +
    'объём — в Examples. Variants — небольшая витрина СОСТОЯНИЙ собранного массива (пусто / заполнено / ' +
    'disabled / invalid / режимы FormArraySection), а не набор визуальных настроек.',
  variants: [
    {
      id: 'filled',
      title: 'Заполнен (базовый)',
      description: 'Дефолтный рендер: карточка на элемент, кнопка удаления и кнопка добавления.',
      render: FormArrayFilled,
  ...
  <FormArray.AddButton initialValue={{ kind: '', number: '' }}>
    ...
    },
    {
      id: 'empty',
      title: 'Пустой массив',
      description: 'length = 0 → рендерится FormArray.Empty, список скрыт.',
      render: FormArrayEmptyState,
      code: `<FormArray.Root control={form.phones /* [] */}>
  <FormArray.Empty>
  ...
  <FormArray.AddButton initialValue={{ kind: '', number: '' }}>
    ...
    {
      id: 'disabled',
      title: 'Отключённый массив',
      description:
        'После arrayNode.disable() массив структурно неизменяем — AddButton и RemoveButton авто-отключены.',
      hint: 'disabled наследуется от узла массива, а не задаётся на кнопках',
      render: FormArrayDisabled,
      code: `// массив помечен неизменяемым
form.phones.disable();

<FormArray.Root control={form.phones}>
  <FormArray.List>{(item) => <PhoneCard {...item} />}</FormArray.List>
  {/* AddButton / RemoveButton авто-disabled */}
  <FormArray.AddButton>+ Добавить телефон</FormArray.AddButton>
</FormArray.Root>`,
    },
    {
      id: 'invalid',
      title: 'Ошибка уровня массива',
      description:
        'control.errors непусто (напр. minItems) → под списком выводится FormArray.Error.',
      hint: 'invalid-состояние массива (не отдельного поля)',
      render: FormArrayInvalid,
      code: `// array-level ошибка
form.phones.setErrors([
  { code: 'minItems', message: 'Добавьте хотя бы один телефон' },
]);
...
<FormArray.Root control={form.phones}>
  ...
  <FormArray.AddButton>+ Добавить телефон</FormArray.AddButton>
    ...
    {
      id: 'section-title',
      title: 'С заголовком (FormArraySection)',
      description: 'title задан → заголовок секции и кнопка добавления в шапке.',
...
/>`,
    ...
    {
      id: 'section-no-title',
      title: 'Без заголовка (footer-ссылка)',
      description: 'title не задан → кнопка добавления рендерится ссылкой-футером внизу.',
...
/>`,
    },
    {
      ...
      title: 'С переупорядочиванием',
      description:
        'reorderable=true → на каждой карточке кнопки ↑/↓ (canMoveUp/canMoveDown → disabled).',
...
/>`,
    },
    {
      ...
      title: 'Лимит достигнут (maxItems)',
      description: 'length ≥ maxItems → кнопка добавления скрывается.',
      hint: 'maxItems={2} и уже 2 элемента',
...
/>`,
    ...
    {
      id: 'section-remove-single',
      title: 'Удаление скрыто на последнем',
      description: 'showRemoveOnSingle=false (default) + один элемент → кнопка «Удалить» скрыта.',
      hint: 'политика по умолчанию для единственного элемента',
  ...
  title="Телефоны"
  itemLabel="Телефон"
  // showRemoveOnSingle={false} — default: при 1 элементе «Удалить» скрыт
  initialValue={{ kind: '', number: '' }}
/>`,
    },
  ],
  examples: [
    {
      id: 'headless',
      title: 'Headless-сборка из частей',
      description:
        'Root + Count + Empty + List + ItemIndex + RemoveButton + AddButton. Разметку и стили пишете сами.',
