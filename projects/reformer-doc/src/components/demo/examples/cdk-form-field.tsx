...
// ─── Общая cdk-анатомия для витрины состояний (Variants) ────────────────────
// Один headless-скелет Root/Label/Control/Error(+Description), состояние которого
// целиком задаёт FieldNode. Разные пресеты передают в него control из useDemoField.
function Field({
  control,
  description,
  multiError = false,
...
// ─── Variants: витрина состояний собранного поля ────────────────────────────
...
function RequiredField() {
  const { control } = useDemoField({
    ...
    validators: [required({ message: 'Введите email' })],
  });
  return <Field control={control} />;
}
...
function InvalidSingle() {
  const { control } = useDemoField({
    ...
    validators: [required({ message: 'Введите email' }), email({ message: 'Некорректный email' })],
    touched: true,
  });
  return <Field control={control} />;
}
...
// (5) Свободная композиция / кастомная раскладка — headless не навязывает DOM.
...
// (8) Индикатор async-валидации: читаем pending из контекста и рисуем спиннер/«Проверка…».
...
export const cdkFormFieldDocConfig: ComponentDocConfig = {
  ...
  description:
    ...
    'Variants — витрина состояний собранного поля (пусто / заполнено / disabled / invalid / required), ' +
    'Examples — возможности анатомии (asChild, render-props, хуки, ui-kit-обёртка). UI вы строите сами.',
  variants: [
    {
      id: 'auto',
      title: 'Auto-render (baseline)',
      description:
        'Базовая анатомия: Label и Control берут текст/компонент из ноды схемы, Error прячется до touch.',
      render: AutoRender,
...
<FormField.Root control={form.email}>
  ...
  <FormField.Error />
    ...
    {
      id: 'filled',
      title: 'Заполнено / valid',
      description: 'Собранное поле со значением и без ошибок — валидное состояние.',
      render: FilledValid,
      code: `// value = 'you@example.com', валидаторы проходят
email: {
  value: model.$.email,
  component: Input,
  componentProps: { label: 'Email', type: 'email' },
  validators: [required(), email()],
    ...
    },
    {
      ...
      title: 'Обязательное',
      description:
        'componentProps.required=true: Label добавляет маркер « *», Control получает aria-required.',
  ...
  validators: [required({ message: 'Введите email' })],
    ...
    {
      id: 'invalid',
      title: 'Invalid (одна ошибка)',
      description:
        'touched + проваленный валидатор: показана первая ошибка, на Control выставлены aria-invalid / aria-errormessage.',
      render: InvalidSingle,
      code: `<FormField.Root control={form.email}>
  <FormField.Label />
  <FormField.Control />
  {/* touched-поле с проваленным email() → Error виден */}
  <FormField.Error />
</FormField.Root>`,
    },
    {
      id: 'disabled',
      title: 'Disabled',
      description: 'control.disable(): Control рендерится disabled, связка id/aria сохраняется.',
      render: DisabledField,
      code: `const control = form.email;
control.disable();
...
<FormField.Root control={control}>
  ...
  <FormField.Control />
    ...
    },
    {
      ...
      title: 'С описанием',
      description:
        'hasDescription + FormField.Description: helper-текст под контролом, aria-describedby авто-связан.',
  ...
  <FormField.Error />
    ...
    },
    {
      ...
      description:
        'Error multi=true: перечислены все ошибки поля; id/aria-errormessage — на первой.',
...
/>`,
    ...
    {
      id: 'custom-layout',
      title: 'Свободная композиция / раскладка',
      description:
        'Горизонтальный grid или любая своя структура частей — headless не навязывает DOM, id/aria провязываются в любом порядке.',
      render: CustomLayout,
      code: `<FormField.Root control={form.email} hasDescription>
