...
│   │   │                    в `boot({ application })` из main.tsx
│   │   └── composer/
│   │       └── builtin-plugins.ts  единственное место со списком встроенных плагинов;
│   │                        здесь же шесть литеральных `import()` ленивой фазы
│   │
...
- **`application/composer/builtin-plugins.ts` — единственное место со списком плагинов.**
  В оболочке его нет и быть не может: она получает состав параметром.
...
- `application/composer/builtin-plugins.ts` — шесть литеральных `import('@/plugins/<id>')`
  в `loadLazyBuiltinPlugins`: сборщику нужен литерал, переменной путь не задать. Список
  `LAZY_PLUGIN_IDS` рядом — для проверок, и его расхождение с телом функции ловит тест состава.
