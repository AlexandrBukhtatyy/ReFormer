...
| `plugins/<домен>/core/` | ядро домена: чистый предметный код, общий плагинам домена  | контракт плагинов и библиотеки; без React           |
| `plugins/<домен>/<п>/`  | плагин: предметная логика как вклады                       | контракт плагинов, ядро СВОЕГО домена, свой каталог |
| `shell/boot/`           | сборка оболочки: платформа, порты, службы, запуск          | всё, кроме `application/` и ядер доменов            |
| `application/`          | СОСТАВ приложения: из каких плагинов оно собрано           | всё                                                 |

Предметный код, общий для плагинов одного домена (стека), живёт ядром этого домена —
`plugins/<домен>/core/`: у ReFormer там модель схемы, каталог, запасные значения китов, моки,
фикстуры и печать модуля формы для `@reformer/renderer-json`; у демо-стека `plain` — формат схемы,
операции, проверки и печать. Пакетом остаётся только нейтральное: `@reformer/builder-toolkit` — то,
что нужно любому стеку (печать шаблонов, маркер происхождения, имена). До 2026-09 ядра были
пакетами `@reformer/builder-stack-*` в `packages/`, а ещё раньше — каталогом `src/lib`, который
...
1. **`shell/platform/` не импортирует `plugins/`** (ни плагины, ни ядра доменов). Это буквальное
   выражение принципа «платформа не знает предметных сущностей». Нарушение видно сразу.
2. **`plugins/*` не импортируют друг друга.** Связь — через сервисы, команды и события. Из
   `@/plugins` плагину видно только ядро СВОЕГО домена (`@/plugins/<домен>/core`); чужой домен —
   только через SDK (точки расширения, возможности с версиями).
3. **Ядро домена не знает билдера.** Ни `@/shell`, ни `@/application`, ни плагинов — ни своих, ни
   чужих; ни React: ядро — формат, операции, проверки и печать, интерфейс — работа плагинов.
   Из SDK — только контракт (чистые функции контракта кита и типы), без `/internal`.
...
5. **`shell/` не знает стека**: не импортирует ядра доменов и плагины, которых нет
...
Поэтому: **чистое и без состояния — в ядре домена, импортируется напрямую. Состояние
...
нужна никому больше. Соседний домен её не видит — линтер открывает плагину только ядро СВОЕГО
домена. То, что нужно ЛЮБОМУ стеку, — в `@reformer/builder-toolkit`; критерий туда — «нужно двум
стекам», а не «не знает про ReFormer».
...
| `catalog`      | ReFormer-проекция каталога кита (`projectCatalog`): палитра, инспектор, фабрики узлов              |
| `kits`         | запасные значения для вызовов без дескриптора активного кита (листья, инфраструктура)              |
...
| `testing.ts`   | фикстуры для тестов потребителей (схема-образец, встроенный каталог, киты) — отдельно от `catalog` |

Состояние здесь не живёт: «какой кит активен», «какой документ открыт», «что выделено»
принадлежит плагинам и службам — в ядре только то, что следует из данных.
...
Правила доменов линтер строит по каталогам `src/plugins/*`: новый домен получает свои блоки
(«плагину видно только своё ядро», «ядро без React и оболочки») без правки конфига.
...
| v1                                 | v2                                                       |
| ---------------------------------- | -------------------------------------------------------- |
| `model/`                           | `reformer/core/form-model/`                              |
| `catalog/`                         | `reformer/core/catalog/`                                 |
| `kits/` (дескрипторы)              | SDK `src/kits/` + `reformer/core/kits/`                  |
| `kits/` (активный кит)             | `plugins/kits/registry/`                                 |
| `preview-runtime/live/`            | `shell/platform/modules/` + `plugins/reformer/render/`   |
| `codegen/`                         | `reformer/core/codegen/` + `plugins/reformer/codegen/`   |
