 ...
 *
 * `Tree` в этом списке при роли `container` не противоречие: роль говорит «узел не поле формы», а
 * лист — «детей внутрь не положить». Строки дерева приходят из пропа `nodes` (данные), а `children`
 * у него нет вовсе: без этой записи палитра дала бы Tree drop-зону, куда всё падало бы в никуда,
 * а `makeNode` пришил бы ему `className: 'space-y-4'` и пустой `children`.
 */
