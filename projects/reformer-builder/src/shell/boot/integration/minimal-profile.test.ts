 ...
 *
 * Вопрос не праздный. `boot` до сих пор берёт из плагинов пять значений (реестр состояний
 * превью, тело Monaco, токен китов, словарь и идентификатор файлов) и собирает порты для ВСЕХ
 * встроенных, включая тех, кого профиль не назвал. Порт без потребителя обязан оставаться
