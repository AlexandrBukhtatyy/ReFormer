 ...
 * ## Что здесь НАСТОЯЩЕЕ
 *
 * Всё, кроме браузерных хранилищ: реестр модулей с подлинным `@reformer/builder-plugin-api`, загрузчик, каталог
...
describe('внешний плагин каталога как редактор кода', () => {
  ...
  it('его команда правит текст документа службой документов', async () => {
    ...
    // ни прочитать текст, ни записать его: в `@reformer/builder-plugin-api` не было ни одного такого имени.
    const h = harness();
    const session = await h.start();
  ...
  it('словарь, отданный ЕГО КОДОМ, попадает в его же пространство имён', async () => {
    ...
    // некуда — сервиса локализации в `@reformer/builder-plugin-api` не было вовсе.
    const h = harness();
    await h.start();
    await h.i18n.setLocale('ru');
