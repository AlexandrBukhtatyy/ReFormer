...
export interface PluginRegistry {
   ...
   *
   * Объявлений не принимает намеренно: набор без объявлений — это набор, у которого их нет
   * (тесты, отдельно собранный плагин). Тот, у кого они есть, регистрирует по одному —
   * см. `shell/boot/boot`, где пара «плагин + объявление» приходит из состава приложения.
   */
