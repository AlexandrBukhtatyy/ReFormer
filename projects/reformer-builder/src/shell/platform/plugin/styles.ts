 ...
 * идентификатор.
 *
 * Ставит его оболочка, и ровно в одном месте — компонент `ui/chrome/PluginScope`, которым
 * обёрнуто тело КАЖДОГО вклада (панели доков и рейла, панели центра, ячейки строки состояния,
 * тело редактора). Владелец берётся из `Contribution.pluginId`, поэтому назвать чужой скоуп
 * плагину нечем. Пока такой обёртки не было, переписанная здесь таблица не совпадала ни с чем.
 *
