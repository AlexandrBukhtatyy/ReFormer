...
export interface SettingField {
  ...
  /**
   * Из какого СЛОЯ пришло действующее значение. Необязательно.
   *
   * Слои без указания источника превращаются в «почему у меня не применяется»: человек
   * меняет глобальную настройку, она перекрыта настройкой проекта, и на экране ничего
   * не происходит. Окно показывает пометку ровно тогда, когда перекрытие есть, — то есть
   * когда ответ `'workspace'`.
   *
   * Необязательно, потому что не у всякого поля есть слои: у настройки, живущей не в службе
   * настроек, отвечать на этот вопрос нечем, и молчание здесь честнее выдуманного ответа.
   */
  origin?(): 'user' | 'workspace' | 'default' | undefined;
}
