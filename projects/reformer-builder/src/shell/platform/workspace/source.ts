 ...
 *
 * Всё, чего здесь нет, Workspace не делает: он не удаляет и не переименовывает — эти
 * операции приходят отдельными командами поверх полного `Source`. Каталог под НОВЫЙ файл
 * ({@link WorkspaceSource.mkdir}) — исключение, и объяснено оно там же: это не операция
...
export interface WorkspaceSource {
  ...
  /**
   * Создаёт каталог со всеми недостающими уровнями.
   *
   * Нужен ровно для одного: {@link WorkspaceSource.write} пишет ФАЙЛ, а не путь, и файловый
   * источник на несуществующем каталоге отвечает `not-found`. Материализовать файл по адресу
   * внутри ещё не созданного каталога при этом законно и обычно — так появляется любой новый
   * модуль (форма по шаблону, вывод кодогена, фикстура в отдельном дереве). Без этого метода
   * такая запись доходит только до рабочей копии, а отправка в источник отказывает — молча
   * для всех, кто смотрит на «сохранено» вместо `SaveResult.failures`.
   *
   * Необязателен: у источника с плоским пространством имён каталогов нет вовсе, и создавать
   * ему нечего — там запись по любому пути проходит и так.
   */
