...
describe('стек не знает ReFormer', () => {
  it('модули ядра не импортируют рендерер, ядро форм и домен ReFormer', () => {
    // Демо-стек доказывает, что швы билдера пропускают НЕ-ReFormer стек. Раньше это стерёг
    // package.json отдельного пакета; ядро живёт в билдере, и проверяются сами импорты.
    const sources = import.meta.glob<string>(['./*.ts', '!./*.test.ts'], {
      query: '?raw',
      import: 'default',
      eager: true,
    });
    const specifiers = Object.values(sources).flatMap((source) =>
      [...source.matchAll(/from '([^']+)'/g)].map((match) => match[1]!)
    );

    const foreign = specifiers.filter(
      (specifier) =>
        /^@reformer\/(renderer-json|core)(\/|$)/.test(specifier) ||
        specifier.startsWith('@/plugins/reformer')
    );
