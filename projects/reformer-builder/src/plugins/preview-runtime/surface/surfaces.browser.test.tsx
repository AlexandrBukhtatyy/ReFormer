 ...
 *
 * @module plugins/preview-runtime/surface/surfaces.browser.test
 */
...
function fakeContext(schema: JsonFormSchema | null): FakeContext {
  ...
  // Документ формы — модельный, и разобрал его провайдер схемы формы: по нему поверхности
  // стека узнают свой документ (`.json` бывает схемой любого стека).
...
const surfaces = builtinSurfaces(createFakeHost(), (key) => key);
...
describe('компилирующая поверхность и правка схемы', () => {
  ...
  it('правка схемы пересобирает форму, но не запускает компиляцию заново', async () => {
    ...
    const compiling = builtinSurfaces(createFakeHost({ modules, siblings }), (key) => key).find(
