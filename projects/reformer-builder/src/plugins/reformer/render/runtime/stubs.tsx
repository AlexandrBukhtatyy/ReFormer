 ...
 *
 * @module plugins/reformer/render/runtime/stubs
 */
 ...
 * рисовать ли подпись рядом и на каком диалекте он говорит со значением. Потеряй обёртка любой
 * из них — подпись у Checkbox задвоилась бы, а поле осталось бы без сигнала.
 *
 * `reformerAdapter` — диалект контрола (`checked` вместо `value`, `onChange(event)` вместо
 * `onChange(value)`). Рендерер ищет его на том компоненте, что лежит в реестре, то есть на
 * обёртке; без копии он связывает поле value-based seam'ом, и `Input` пишет в модель DOM-событие.
 * С 19.09 адаптеры контролов кита — именно статики (`defineFieldControl`), и список обязан
 * расти вместе с ними: это стережёт храповик в `./stubs.test`.
 */
export const CONTRACT_STATICS: readonly string[] = [
