 ...
 *   рамку кита и его происхождение служба отдаёт как есть.
 *
...
export function previewHostFromContext(ctx: Pick<PluginContext, 'services' | 'i18n'>): PreviewHost {
  ...
  return {
    ...
    kitOrigin: () => kit()?.activeOrigin() ?? null,
    // Рамка стабильна на всё время жизни службы: пока служба та же, форма не размонтируется.
    kitFrame: () => kit()?.Frame ?? null,
