 ...
 * приходит ПАРАМЕТРОМ — тем же приёмом, каким объявляют свою потребность `plugins/base/files/host`,
 * `plugins/base/editor-monaco/host` и `plugins/reformer/editor/host`: потребность объявляет потребитель,
 ...
 * каталог отвечает «какие имена бывают» (данные, ReFormer-проекция `@/plugins/reformer/core/catalog`),
 * дескриптор — «как их резолвить» (данные), namespace — «чем рисовать» (значения, React-компоненты).
 * Namespace едет лениво и может не доехать вовсе, поэтому он необязателен, а рантайм-поверхность
 * без него штатно деградирует в подписанные стабы.
 *
 * Ещё две вещи — не про компоненты, а про ОКРУЖЕНИЕ формы: рамка кита (скоуп стилей плагина-
 * владельца и провайдер кита) и происхождение кита. Форма кита, внесённого плагином, рисуется
 * внутри рамки — иначе остаётся без своих стилей и темы; а код формы, импортирующий пакет такого
 * кита, получает вместо пакета его namespace — в реестре модулей оболочки этого пакета нет.
 *
 * Всё это — ФУНКЦИИ, а не значения: кит переключают без перезагрузки, и захваченный в замыкание
...
export interface PreviewDocument {
  ...
  /**
   * Провайдер модели, который взялся за документ; есть только у модельного. По нему превью-хост
   * выбирает поверхность: `.json` бывает схемой любого стека.
   */
...
export interface PreviewHost {
  ...
  /**
   * Откуда активный кит: встроен или внесён плагином. `null` — службы китов нет.
   *
   * Нужен компилирующей поверхности: пакет встроенного кита оболочка отдаёт коду формы своим
   * реестром модулей, а пакет кита плагина — нет, и его импорт подставляется namespace.
   */
  kitOrigin(): KitOrigin | null;

  /**
   * Рамка активного кита — окружение, в котором рисуется форма: скоуп стилей плагина-владельца
   * и провайдер кита. Компонент стабилен и сам следит за сменой кита. `null` — службы китов нет,
   * и форма рисуется без рамки.
   */
  kitFrame(): ComponentType<KitFrameProps> | null;
