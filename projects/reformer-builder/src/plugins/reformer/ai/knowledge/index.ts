 ...
 *
 * @module plugins/reformer/ai/knowledge/index
 */
...
async function projectKnowledge(files: PackageFiles): Promise<KnowledgeSource | null> {
  try {
    ...
    console.warn('[plugins/reformer/ai] не удалось прочитать @reformer/* из проекта', error);
    return null;
  }
}
...
async function bundledKnowledge(): Promise<KnowledgeSource | null> {
  try {
    ...
    console.warn(
      '[plugins/reformer/ai] корпус знаний не загружен — справка по библиотеке недоступна. ' +
