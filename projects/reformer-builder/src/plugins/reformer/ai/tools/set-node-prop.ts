...
import { toInspectorProps } from '@/plugins/reformer/core/catalog';
import { setComponentProp, setTextChild, textChildIndex } from '@/plugins/reformer/core/form-model';
import { catalogEntry, type Catalog } from '../model/catalog-digest';
import { isLeafComponent, kindOf } from '@/plugins/reformer/core/form-model';
  ...
  run(params, ctx) {
    ...
    for (const ref of params.refs) {
      ...
      for (const [key, value] of Object.entries(params.props)) {
        const step =
          key === TEXT_KEY
            ? setText(ctx.catalog, draft, ref, { node, path }, value)
 ...
 */
function captionPropOf(catalog: Catalog, node: JsonNode): string | undefined {
  const name = componentOf(node);
  const entry = name ? catalogEntry(catalog, name) : undefined;
...
function setText(
  ...
  const caption = captionPropOf(catalog, found.node);
  if (caption) {
    return {
      error: fail(
        'INVALID_PARENT',
