 ...
 *
 * @module plugins/reformer/editor/plugin.test
 */
...
describe('activate', () => {
  ...
  it('всё зарегистрированное складывается в подписки — иначе выключение ничего не снимет', () => {
    ...
    // добавление. Затем живая форма — команда и кнопка. Последним — быстрое исправление
    // чтения модели (`schema.set-model-read`). Затем «разбить по шагам» и «собрать в один файл».
    expect(ctx.subscriptions).toHaveLength(38);
  });
});
