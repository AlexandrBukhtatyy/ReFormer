 ...
 *
 * @module plugins/reformer/editor/ui/Canvas
 */
...
export function Canvas({
  ...
  const onKeyDown = useCallback(
    (event: KeyboardEvent<HTMLDivElement>) => {
      ...
      // (`plugins/reformer/editor/commands`). Курсор её не трогает: иначе одно нажатие
    ...
    return (
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <LiveView
          ...
          live={live}
          drag={drag}
          // Тот же свод, что у меток на строках дерева: находка обязана выглядеть одинаково,
          // чем бы ни был показан документ.
          problems={problems}
        />
      </div>
    );
