 ...
 * зашита в раскладку.
 *
 * @module plugins/reformer/editor/plugin
 */
 ...
 *
 * Заведомо выше единицы текстового редактора (`plugins/base/files`): тот берётся за всё, что
...
export interface SchemaEditorPluginOptions {
   ...
   *
   * Подставляется композицией: `@reformer/builder-plugin-api` её пока не отдаёт, а импортировать `@/shell` плагину
   * нельзя — тот же приём, что у точек панели и редактора в `plugins/base/files`.
   */
  /**
   * Точка провайдеров модели.
   *
   * Остаётся параметром, а не берётся импортом из `@reformer/builder-plugin-api` прямо здесь, по той же причине,
