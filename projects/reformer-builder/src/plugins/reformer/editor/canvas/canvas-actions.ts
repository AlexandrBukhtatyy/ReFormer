 ...
 * состояния и хранятся раздельно ({@link '../session/canvas-prefs'} и {@link '../session/view-mode'}): первое
 ...
 *
 * @module plugins/reformer/editor/canvas/canvas-actions
 */
...
export interface CanvasActionDeps {
  ...
  /**
   * Сигнал «состав сеансов изменился» — тот самый реестр, по которому отвечает
   * {@link CanvasActionDeps.isSchema}.
   *
   * Сеанс заводит ЭФФЕКТ тела редактора, то есть уже после того, как полоса вкладок
   * отрисована. Видимость кнопок это не задевает (см. {@link CanvasActionDeps.editorId}),
   * а вот их ДОСТУПНОСТЬ — задевает: команда отвечает по `isSchema`, ряд рисуется до сеанса
   * и получает «не схема», а перерисовать его после этого было нечем — так на только что
   * открытой форме все четыре кнопки стояли выключенными до первой чужой перерисовки.
   * Необязателен ради встраиваний без реестра; плагин отдаёт его всегда.
   */
  readonly sessions?: { subscribe(listener: () => void): Disposable };
  /**
   ...
   * редактора, то есть уже после того, как полоса вкладок отрисована. Редактор же известен
   * синхронно: его выбирает оболочка ещё до отрисовки ряда и приносит в цели, — поэтому
   * видимость кнопки не зависит от того, успел ли сеанс появиться.
   */
...
export function canvasViewMenuItems(
  ...
  // Третий источник сигнала — сеансы: `isSchema` отвечает по ним, а сеанс рождается в эффекте
  // тела редактора, то есть ПОСЛЕ того, как ряд кнопок над ним уже нарисован. Без этой подписки
  // ряд застывал в положении «не схема» (все кнопки выключены) до первой чужой перерисовки —
  // ровно так он и выглядел у только что открытой формы.
  const signal = (cb: () => void): Disposable => {
    const subscriptions = [prefs.subscribe(cb), views?.subscribe(cb), deps.sessions?.subscribe(cb)];
    return {
      dispose: () => {
