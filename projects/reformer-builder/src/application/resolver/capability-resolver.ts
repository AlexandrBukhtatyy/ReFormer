 ...
 *
 * Снять конфликт можно ВЫБОРОМ ({@link ResolveCapabilitiesOptions.chosen}). Поле профиля
 * `providers`, которое этот выбор понесёт, — работа фазы 4: пока выбор передаётся параметром,
 * потому что придумывать формат хранения раньше первого настоящего конфликта значило бы
 * угадывать.
 *
...
export interface MissingCapability {
   ...
   *
   * Без этого списка сообщение «нужна kits.active@^2» не отвечает на первый же вопрос
   * человека: «а какая есть?». Ответ «1.0.0 от плагина kits» отличает «поставь новее»
   * от «поставь вообще».
   */
