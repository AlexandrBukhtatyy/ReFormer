...
Уровень запуска читает лаунчер (`bin/reformer-builder2.mjs`, каталог, где сделали
`npx reformer-builder2`); уровень проекта читается самим приложением из ОТКРЫТОГО проекта.
