#!/usr/bin/env node
/**
 * reformer-builder2 — локальный запуск визуального билдера ReFormer (v2).
 *
 * Опубликованный пакет несёт УЖЕ собранный SPA в `dist/`. Этот скрипт — zero-dependency
 * статик-сервер на встроенных модулях Node (перенос launcher'а v1, `projects/reformer-builder/
 * bin/reformer-builder.mjs`): поднимает `dist/` на localhost и открывает браузер. Серверной
 ...
 *
 * Каталога компонентов (`--catalog` из v1) здесь НЕТ намеренно: сторонний ui-kit в v2
 * встраивается плагином (`.ui_builder/plugins/`), а не файлом данных — плагин привозит
 * и каталог, и код компонентов.
 *
 * Использование:
 *   npx reformer-builder2 [--port <n>] [--host <h>] [--no-open] [--config <path>]
 */
...
async function readRuntimeFile(path, { explicit }) {
  ...
  try {
    ...
    if (explicit) {
      console.error(`reformer-builder2: не удалось прочитать конфиг "${path}": ${err.message}`);
      process.exit(1);
    }
    return null;
  }
  try {
    ...
    console.error(`reformer-builder2: невалидный JSON в конфиге "${path}": ${err.message}`);
    process.exit(1);
  }
}
...
export function parseArgs(argv) {
  const opts = {
    // 4322, а не 4321: билдеры v1 и v2 живут рядом, и дефолтные порты не должны толкаться.
    port: 4322,
    host: '127.0.0.1',
    open: true,
    help: false,
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--help' || a === '-h') opts.help = true;
      ...
      console.error(`reformer-builder2: неизвестный аргумент "${a}" (см. --help)`);
      process.exit(1);
    }
  }
  if (!Number.isInteger(opts.port) || opts.port < 0 || opts.port > 65535) {
    console.error(`reformer-builder2: некорректный --port (ожидается 0..65535)`);
    process.exit(1);
  }
  return opts;
}
...
function printHelp() {
  console.log(`reformer-builder2 — визуальный билдер ReFormer-схем (локальный запуск)

Использование:
  npx reformer-builder2 [опции]

Опции:
  -p, --port <n>       Порт (по умолчанию 4322; занят — берётся следующий свободный)
...
async function main() {
  ...
  try {
    ...
    console.error(
      `reformer-builder2: не найден собранный dist/ (ожидался ${indexHtmlPath}).\n` +
  ...
  try {
    ...
    console.error(`reformer-builder2: не удалось занять порт: ${err.message}`);
    process.exit(1);
  }

  const displayHost = opts.host === '0.0.0.0' ? 'localhost' : opts.host;
  const url = `http://${displayHost}:${port}/`;
  console.log(`\n  reformer-builder2 v${await readVersion()}`);
...
if (process.env.REFORMER_BUILDER_TEST !== '1') {
  main().catch((err) => {
    console.error(`reformer-builder2: ${err?.stack || err}`);
    process.exit(1);
  });
}
