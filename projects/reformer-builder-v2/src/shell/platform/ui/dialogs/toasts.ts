/**
 * Показ уведомлений тостами — всё, что в нём является правилом, а не отрисовкой.
 *
 * Служба уведомлений (`shell/platform/services/notifications`) держит ОЧЕРЕДЬ не показанных
 * запросов и намеренно ничего не знает ни про тосты, ни про таймеры. Здесь — вторая половина:
 * как эта очередь превращается в показ и почему она при этом пустеет.
 *
 * Имя файла — `toasts`, а не `notifications`, хотя предмет тот же: одноимённый модуль
 * в службах отвечает на другой вопрос, и две пары `notifications.ts` + `notifications.test.ts`
 * в разных каталогах различались бы только путём — то есть в поиске по имени не различались бы
 * вовсе. Здесь тосты, там очередь.
 *
