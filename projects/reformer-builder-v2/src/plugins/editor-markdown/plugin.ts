 ...
 * Приоритет редактора.
 *
 * Выше текстового (у того единица), потому что для markdown рендер — это то, зачем файл
 * открывают чаще всего. Ниже редактора схемы формы: `.md` схемой не бывает, но правило
 * «предметный редактор важнее общего» должно читаться из чисел без оговорок.
 */
export const MARKDOWN_EDITOR_PRIORITY = 10;
...
export function markdownMenuItems(
  ...
  const onMarkdown = whenEditor((target) => isMarkdown(target.ref.name, target.ref.mediaType));
