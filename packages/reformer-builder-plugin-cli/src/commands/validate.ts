 ...
 *
 * Чего здесь НЕТ и почему: сверки `provides` с тем, что плагин регистрирует, потолка числа
 * файлов и сверки `compatibility.builder` с версией билдера. Первое требует исполнить код,
 * второе относится к собранному каталогу, а третьего CLI знать не может: в каком билдере плагин
 * запустят, решает не он, и отказ по этому полю был бы отказом по выдуманному числу. Форма
 * диапазона при этом проверяется — опечатка ловится здесь, а не у человека при загрузке.
 *
