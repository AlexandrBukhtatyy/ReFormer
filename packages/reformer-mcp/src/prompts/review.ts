...
export function getReviewPrompt(args: { code: string }): {
  ...
  const faqAll = getTroubleshooting('*');
  const antiPatternsAll = getSection('Anti-patterns', '*');
  const reactIntegration = getSection('React Integration', '@reformer/core');
    ...
    messages: [
      {
        ...
        content: {
          type: 'text',
          text: `You are a senior reviewer for ReFormer-based forms. Audit the supplied code against the cross-package conventions below.
