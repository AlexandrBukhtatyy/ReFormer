...
- **NEVER combine `model:` with a container `component`.** `model:` is the discriminator for «leaf field» — converter ignores `component` and treats the node as a field bound to that path. If you need a container that is **bound to a form path** (FormArraySection, custom array UIs), drop `model:` and pass the path through `componentProps.control: '<path>'` instead. The converter's string-resolver handles `control` / `*Control` componentProps automatically (resolves string → FieldPath / FormProxy).
  ...
  ```jsonc
  // ❌ silent fail — `model: 'properties'` makes this a FIELD bound to `properties`,
  // FormField wraps the ArrayNode → `componentProps.value` is undefined → `TypeError: Cannot read properties of undefined (reading 'label')` at FormField.Root mount.
  {
    "selector": "properties-section",
    "model": "properties",
    "component": "FormArraySection",
    "componentProps": { "title": "Имущество", "itemComponent": { "$template": { ... } } }
  }

  // ✅ no `model:` → container path; `componentProps.control` carries the form path,
  // converter resolves the string → FormArraySection receives a real ArrayNode.
  {
    "selector": "properties-section",
    "component": "FormArraySection",
    "componentProps": {
...
- [ ] No node combines `model:` with a container `component:` (FormArraySection / custom container) — bound containers get the path via `componentProps.control: '<path>'`
- [ ] `version: '1.0'` present
