...
- **Arrays via `{ array, component, item.$template }`**: `{ "array": "$model(properties)", "component": "$component(FormArray)", "initialValue": { … }, "componentProps": { "title": "…", "itemLabel": "$fn(…)" }, "item": { "$template": { …JsonNode… } } }`. `array`, `component` **and** `item.$template` are all required. Inside `$template`, `$model(...)` paths resolve **relative to the array element** (`"$model(type)"`, not `"$model(properties[0].type)"`). Pick the component by intent: `$component(FormArray)` for an editable section (add/remove/reorder chrome), `$component(List)` for a read-only display list (no `initialValue` needed) — both from `@reformer/ui-kit`, register them with `reg.component`. The renderer itself ships no markup: omit `component` and the items still render, but with no add/remove UI (and a console warning).
...
4. Keep behavior as TS — pass its factory in the `renderBehavior` field of `createJsonForm`, and mount the resulting bundle with `<JsonFormRenderer form={jsonForm} />` inside `JsonRendererProvider`.
...
- [ ] Behavior stays TS, passed as the `renderBehavior` field of `createJsonForm` (bundle → `form` prop)
