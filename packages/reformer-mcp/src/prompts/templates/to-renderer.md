...
- Field node (leaf): `{ value: model.$.<field>, component: InputField, componentProps: { ... } }` — `value` is the **model signal** (`model.$.<field>`), `component` is the UI component by reference.
