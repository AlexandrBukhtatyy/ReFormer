...
- Directory layout — `find_recipe directory-layout` (раскладка и **имена** файлов формы для `{{target}}`; читать ДО написания первого файла — переименовывать потом дороже)
  {{rendererPrereqs}}
...
- **Deferred** — отложено (укажи причину: нужен API не описанный в спеке; требует `validateAsync`-правила с реальным endpoint'ом; UI-feature вне core-промптов; и т.д.).
...
| 3   | **FormSchema + UI**    | `create-form` (`target={{target}}`) | файл схемы по канону раскладки: `form.schema.ts` (core) / `renderer.schema.ts` (renderer-react и renderer-json; `.tsx` — если в схеме JSX) + `index.tsx` со ВСЕМИ шагами инлайн; renderer-json дополнительно `registry.ts`. Полный набор — `find_recipe directory-layout` | Playwright: рендер {{stepsCountText}} step section, screenshots, 0 console errors, все спека-поля имеют `[data-testid^="input-"]`.                 |
| 4   | **Validation**         | `add-validation`                    | отдельный `validation.ts` — `defineValidationSchema<T>(({ model }) => …)` (операторы `validate`/`validateAsync`/`validateWhen`/`cross`/`each`/`apply`) + wizard/submit-конфиг поверх раннера `validateModel`; layout (файл схемы / JSON DSL) НЕ несёт validators | Playwright: empty submit → specific Russian errors совпадают с canonical messages из спеки. Generic `"Поле обязательно для заполнения"` = failure. |
| 5   | **Behaviors**          | `add-behavior`                      | `form.behavior.ts` — `defineFormBehavior<T>(...)`, подключается полем `behavior:` в сборке формы (`createCoreForm`/`createReactForm`/`createJsonForm`); render-поведение renderer-* — `renderer.behavior.ts`                                                       | Playwright: trigger каждый behavior из секции 5 ниже, assert effects (computed cascade, copyFrom, hide-on-condition).                              |
...
- [ ] Sub-agent stage 4 (Validation) держит правила в отдельной `ValidationSchema<T>` (`defineValidationSchema` + `validate`/`cross`/`validateWhen`/`each`/`apply`), прогоняет их раннером `validateModel` (wizard — `validateStep/validateAll`); layout без validators; canonical messages через `required({ message })` у каждого built-in.
