...
| 4   | **Validation**         | `add-validation`                    | отдельный `validation.ts` — `defineValidationSchema<T>(({ model }) => …)` (операторы `validate`/`validateAsync`/`validateWhen`/`cross`/`each`/`apply`) + wizard/submit-конфиг поверх раннера `validateModel`; layout (`schema.ts` / JSON DSL) НЕ несёт validators | Playwright: empty submit → specific Russian errors совпадают с canonical messages из спеки. Generic `"Поле обязательно для заполнения"` = failure. |
