...
const FORM_LAYOUT_ENTRY = [
  ...
  '>',
  '> A form module is FLAT: no `lib/` / `schema/` / `components/steps/` nesting. Wizard steps live',
  '> inline in `index.tsx` OR one folder per step `steps/<slug>/` (kebab slug of the step title, no',
  '> number) with the aggregator `steps/index.ts`; inside a step folder the names are the same.',
  '> Naming rule, identical for every target: `form.<role>` is a form artifact whose suffix names',
  '> the role; every other file is plain-named (`index.tsx`, `types.ts`, `model.ts`,',
  '> `data-sources.ts`, `api.ts`):',
  '>',
  '> - **schema** — `form.schema.ts` (`.tsx` when the schema contains JSX). For renderer-json a',
  '>   plain `form.schema.json` is an accepted variant, but it loses compile-time checking of',
  '>   `$model(...)` paths that `defineJsonSchema<T>` gives.',
  '> - **behavior** — `form.behavior.ts` (model behavior, every target) + `form.render.ts`',
  '>   (render behavior, both renderers).',
  '> - **validation** — `form.validation.ts` (rules over the model, every target).',
  '>',
  '> renderer-json also has `registry.ts`, and may add an optional `wizard.tsx` shim (the library',
  '> exports no `RendererFormWizard`). The former `renderer.schema.*` / `renderer.behavior.ts` /',
  '> `renderer.wizard.tsx` / `validation.ts` names still work but are reported as outdated.',
  '> File names are not a free choice. Full per-target list: `find_recipe directory-layout`, or',
  '> the "Form directory layout" section below. Check the names you picked with',
  '> `validate_form kind="layout"` — before writing, not after.',
  '',
].join('\n');
