...
| `add-validation`   | `code` (required), `requirements` (required)                                                     | Навесить валидацию по контракту `@reformer/core/validation`: отдельная `defineValidationSchema<T>(({ model }) => …)` + операторы `validate`/`validateAsync`/`validateWhen`/`cross`/`each`/`apply`, прогон внешним раннером `validateModel(model, schema)` (layout валидаторов не несёт). Подбирает built-in/кастомные/async/cross-field правила под требования. Подгружает справочник валидаторов, common-patterns, async-watchfield, common-mistakes. |
