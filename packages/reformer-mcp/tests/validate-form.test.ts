...
import { validateFormTool } from '../src/tools/validate-form';
import { validateCode } from '../src/validate/code';
import { CODE_TITLES } from '../src/validate/codes';
...
describe('validate_form — диспетчер', () => {
  ...
  it('каждый kind требует своих аргументов и говорит каких', async () => {
    expect((await validateFormTool({ kind: 'code' })).content[0].text).toMatch(/`code`/);
    expect((await validateFormTool({ kind: 'behaviors' })).content[0].text).toMatch(/dependencies/);
    expect((await validateFormTool({ kind: 'json-schema' })).content[0].text).toMatch(/schema/);
    expect((await validateFormTool({ kind: 'bundle' })).content[0].text).toMatch(/intent/);
  });
...
describe('validate_form kind=code', () => {
  ...
  it.runIf(hasSymbols)('RF004 — оператор валидации вне своей схемы', async () => {
    ...
    const { diagnostics } = await validateCode(code);
  ...
  it.runIf(hasSymbols)('RF005 — оператор поведения вне своей схемы', async () => {
    ...
    const rf005 = (await validateCode(code)).diagnostics.filter((d) => d.code === 'RF005');
  ...
  it('пустой код не считается ошибкой', async () => {
    expect((await validateCode('')).diagnostics).toEqual([]);
  });
});
