...
import { assemble, estimateTokens } from '../src/context/budget';
import { PROFILES, resolveProfile } from '../src/context/profiles';
import { listSymbolsTool } from '../src/tools/list-symbols';
...
describe('потолки выдачи инструментов', () => {
  it.runIf(hasDocs)('list_symbols без фильтра не отдаёт всю поверхность', async () => {
    const { content } = await listSymbolsTool({});
  ...
  it.runIf(hasDocs)('узкий запрос list_symbols не режется', async () => {
    const { content } = await listSymbolsTool({ nameContains: 'validateWhen' });
  ...
  it.runIf(hasDocs)('крупный рецепт обрезается с указанием, где дочитать', async () => {
    const { content } = await findRecipeTool({ topic: 'cookbook' });
    const text = content[0].text;
  ...
  it.runIf(hasDocs)('короткий рецепт не трогаем', async () => {
    const { content } = await findRecipeTool({ topic: 'multi-step' });
