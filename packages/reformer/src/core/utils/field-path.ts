...
function createFieldPathProxy<T>(basePath: string): FieldPath<T> {
  ...
  return new Proxy({} as any, {
    get(_target, prop: string | symbol) {
      ...
      // Поддержка __fieldPath для обратной совместимости с behavior модулем
      ...
      // Игнорируем служебные свойства JavaScript/Promise
      if (
        prop === 'then' ||
        prop === 'catch' ||
        prop === 'finally' ||
      ...
      return new Proxy(node, {
        get(_nodeTarget, nodeProp: string | symbol) {
          ...
          // Поддержка __fieldPath для обратной совместимости
          if (nodeProp === '__path' || nodeProp === '__fieldPath') return newPath;
