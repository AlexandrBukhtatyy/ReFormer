...
import { ComboboxField, ComboboxMultiField } from '@/components/combobox';
import { DatePickerField } from '@/components/date-picker';
import { FileUploadInputField } from '@/components/file-upload';
import { InputPasswordField } from '@/components/input-password';
import { NativeSelectField } from '@/components/native-select';
...
describe('Select (async): кластер [крестик][(i)] левее шеврона', () => {
  it('значение + clearable + tooltip: крестик раньше иконки, резерв на две иконки', () => {
    const html = renderToStaticMarkup(
      <SelectField value="a" options={OPTIONS} clearable tooltip={TIP} />
    );
  ...
  it('только tooltip: резерв на одну иконку; только крестик — тоже одна', () => {
    expect(
      renderToStaticMarkup(<SelectField value={null} options={OPTIONS} tooltip={TIP} />)
    ).toContain('*:data-[slot=select-value]:mr-6');
    const clearOnly = renderToStaticMarkup(<SelectField value="a" options={OPTIONS} clearable />);
  ...
  it('шеврон остаётся у края: триггер больше не получает pr-9, кластера без иконок нет', () => {
    const html = renderToStaticMarkup(<SelectField value={null} options={OPTIONS} />);
...
describe.each([
  ['Combobox', ComboboxField, 'a'],
  ['ComboboxMulti', ComboboxMultiField, ['a']],
  ['SelectMulti', SelectMultiField, ['a']],
...
describe('InputPassword: (i) левее глаза', () => {
  const field = (props: Record<string, unknown>) =>
    renderToStaticMarkup(<InputPasswordField {...props} />);
...
describe('выключенное поле: свои кнопки гаснут, подсказка остаётся рабочей', () => {
  it('глаз пароля disabled и не реагирует на наведение; иконка-подсказка не disabled', () => {
    const html = renderToStaticMarkup(<InputPasswordField value="secret" disabled tooltip={TIP} />);
  ...
  it('крестик очистки у выключенного Select/Combobox не рендерится, подсказка остаётся', () => {
    for (const html of [
      renderToStaticMarkup(
        <SelectField value="a" options={OPTIONS} clearable disabled tooltip={TIP} />
      ),
      renderToStaticMarkup(
        <ComboboxField value="a" options={OPTIONS} clearable disabled tooltip={TIP} />
      ),
    ]) {
  ...
  it('RadioGroup: подпись варианта гаснет вместе с radio (peer-disabled)', () => {
    const html = renderToStaticMarkup(<RadioGroupField value={null} options={OPTIONS} disabled />);
  ...
  it('скрепка FileUpload input не реагирует на наведение в disabled', () => {
    const html = renderToStaticMarkup(<FileUploadInputField value={null} disabled />);
...
describe('inside-декораторы', () => {
  it('NativeSelect: обёртка w-fit, иконка левее шеврона, резерв pr-16', () => {
    const html = renderToStaticMarkup(
      <NativeSelectField value={null} options={OPTIONS} tooltip={TIP} />
    );
  ...
  it('DatePicker: резерв через has-[>svg]: (обычный pr-* на Button мёртв)', () => {
    const html = renderToStaticMarkup(<DatePickerField value={null} tooltip={TIP} />);
    expect(html).toContain('has-[&gt;svg]:pr-9');
  });

  it('FileUpload input: кластер [крестик][(i)][скрепка], резерв pr-23', () => {
    const html = renderToStaticMarkup(<FileUploadInputField value={null} tooltip={TIP} />);
    expectOrder(html, 'data-slot="info-hint"', 'aria-label="Выбрать файлы"');
    expect(html).toContain('pr-23');
    expect(renderToStaticMarkup(<FileUploadInputField value={null} />)).toContain('pr-16');
  });
});
...
describe('иконка после текста и СНАРУЖИ <label>', () => {
  it('Checkbox: клик по иконке не должен переключать чекбокс', () => {
    const html = renderToStaticMarkup(
      <CheckboxField value={false} label="Согласен" tooltip={TIP} />
    );
    expectOrder(html, 'Согласен', '</label>', 'data-slot="info-hint"');
  });

  it('Switch: иконка после подписи', () => {
    const html = renderToStaticMarkup(<SwitchField value={false} label="Включить" tooltip={TIP} />);
