...
} as const satisfies Record<string, PropsSchema>;

/** Runtime-пропсы FileUpload-вариантов (seam подмешает mergeFieldPropsSchema). */
export const fileUploadSharedRuntimeProps = {
...
} as const satisfies PropsSchema['x-runtimeProps'];
 ...
 *
 * `x-registryName: 'FileUpload'` — кнопка-триггер (`FileUploadBase`). Варианты `FileUploadDropzone` и
 * `FileUploadInput` делят с ним контракт значения и набор пропсов ({@link fileUploadSharedProperties}),
 * различается только визуал.
 */
