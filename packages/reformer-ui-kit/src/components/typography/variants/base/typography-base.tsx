...
// Набор типографских компонентов на Tailwind-классах по образцу shadcn/ui typography
// (https://ui.shadcn.com/docs/components/typography). В shadcn это guide-страница, а не
// registry-компонент: здесь оформлено как переиспользуемые wrapper'ы. Каждый — тонкая обёртка
// над нужным тегом с `data-slot` и мёржем className через cn() (класс вызывающего перекрывает).
