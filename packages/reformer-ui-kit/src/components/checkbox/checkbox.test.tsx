...
import { Checkbox, CheckboxWithLabel } from './index';
...
describe('CheckboxWithLabel (field, inline-label)', () => {
  it('value=true → отмечен (data-state=checked)', () => {
    const html = renderToStaticMarkup(
      <Bound component={CheckboxWithLabel} value={true} label="Согласен" />
    );
    expect(html).toContain('data-state="checked"');
  });
  ...
  it('value=false → снят; null трактуется как false', () => {
    expect(
      renderToStaticMarkup(<Bound component={CheckboxWithLabel} value={false} label="X" />)
    ).toContain('data-state="unchecked"');
    expect(
      renderToStaticMarkup(<Bound component={CheckboxWithLabel} value={null} label="X" />)
    ).toContain('data-state="unchecked"');
  });

  it('рисует подпись СПРАВА от чекбокса (label после Root)', () => {
    const html = renderToStaticMarkup(
      <Bound component={CheckboxWithLabel} value={false} label="Согласен с условиями" />
    );
  ...
  it('data-testid ложится на Root примитива (не на wrapper / bubble-input)', () => {
    const html = renderToStaticMarkup(
      <Bound component={CheckboxWithLabel} value={false} label="X" data-testid="agree" />
    );
  ...
  it('htmlFor↔id связывают <label> и Root (a11y)', () => {
    const html = renderToStaticMarkup(
      <Bound component={CheckboxWithLabel} value={false} label="X" id="ctrl-a" />
    );
  ...
  it('подпись задаёт доступное имя: висячий aria-labelledby сбрасывается', () => {
    const html = renderToStaticMarkup(
      <Bound component={CheckboxWithLabel} value={false} label="X" aria-labelledby="label-a" />
    );
  ...
  it('без подписи aria-labelledby пробрасывается (имя — снаружи)', () => {
    const html = renderToStaticMarkup(
      <Bound component={CheckboxWithLabel} value={false} aria-labelledby="label-a" />
    );
  ...
  it('выставляет статический маркер reformerLayout=inline-label', () => {
    expect((CheckboxWithLabel as { reformerLayout?: string }).reformerLayout).toBe('inline-label');
  });
});
