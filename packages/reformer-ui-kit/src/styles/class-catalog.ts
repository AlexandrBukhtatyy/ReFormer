 ...
 * допустим.
 *
 * ВАЖНО про сборку билдера: этот словарь разворачивается в safelist Tailwind
 * (`projects/reformer-builder/scripts/gen-kit-safelist.mjs`), потому что `className` из схемы формы
 * сканер не видит — он живёт в данных пользователя, а не в коде. Практическое следствие: класс,
 * добавленный здесь, появится в CSS билдера, а удалённый — исчезнет, поэтому список стоит держать
 * курируемым (полный перебор «вариант × утилита» просто раздувает бандл).
 *
...
export const CLASS_GROUPS: ClassGroup[] = [
  ...
  },
  {
    ...
    label: 'Адаптив',
    classes: [
      // Брейкпоинт-варианты РАСКЛАДКИ: без них «одна колонка на телефоне, две на десктопе» —
      // самый частый макет формы — в билдере не набирается вовсе.
      //
      // Список курируемый и намеренно короткий: каждый вариант — отдельное CSS-правило в бандле
      // билдера (словарь уезжает в safelist, см. scripts/gen-kit-safelist.mjs), а полный перебор
      // «брейкпоинт × утилита» дал бы кратный рост CSS ради классов, которыми никто не пользуется.
      // Классы записаны ЛИТЕРАЛАМИ (не `${bp}:${c}`): строку-литерал видит и сканер Tailwind,
      // поэтому в монорепо они генерируются даже без safelist.
      //
      // `xl:`/`2xl:` не включены сознательно: форма к таким ширинам уже не перестраивается,
      // её ограничивает `max-w-*` контейнера.
      'sm:grid-cols-2',
      'sm:grid-cols-3',
      'md:grid-cols-2',
      'md:grid-cols-3',
