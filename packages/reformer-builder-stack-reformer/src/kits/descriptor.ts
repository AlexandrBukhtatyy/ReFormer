 ...
 *
 * @module @reformer/builder-stack-reformer/kits/descriptor
 */
...
export function toDescriptor(json: CatalogJson): KitDescriptor {
  ...
  return {
    ...
    // Перенос COMPOUND_TEMPLATES из `lib/catalog/make-node` в дескриптор — этап 2.
 ...
 * (`Checkbox` → `CheckboxWithLabel`), обязано назвать его само. Соглашение по суффиксу выражало привычку
