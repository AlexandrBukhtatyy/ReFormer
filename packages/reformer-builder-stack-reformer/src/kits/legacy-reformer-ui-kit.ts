 ...
 *
 * Исходные модули v1 (`catalog/contract`, `preview-runtime/render-policy`, `model/node-kind`,
 * `preview-runtime/known-names`, `codegen/ui-kit-imports`) реэкспортировали эти константы, поэтому
 * их публичная поверхность не изменилась — потребители правились на этапе 2.
 *
 * В v2 этот модуль — ЕДИНСТВЕННЫЙ владелец {@link LEAF_COMPONENT_NAMES}: и `lib/catalog/make-node`,
 * и `lib/form-model/node-kind` берут дефолт отсюда, а не держат свою копию. Пока переезжал только
 * `form-model`, константа временно жила в `node-kind`; второй её копии быть не должно, иначе
 * «лист или нет» отвечалось бы по-разному в дропе из палитры и в обходе дерева.
 *
 * Модуль — ЛИСТ графа зависимостей: только `import type`, ничего из `catalog`/`preview-runtime`/
 * `model`. Иначе получается цикл `catalog → kits → render-policy → catalog`. Именно поэтому
 * `lib/form-model/node-kind` может тянуть отсюда ЗНАЧЕНИЕ: во время выполнения модуль не
 * подтягивает ничего.
 *
 * @module @reformer/builder-stack-reformer/kits/legacy-reformer-ui-kit
 */
