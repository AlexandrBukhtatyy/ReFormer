 ...
 *   catalog-JSON; это ДАННЫЕ, поэтому он и живёт в домене;
 * - **реестр китов** — «между какими китами пользователь может выбирать» плюс ответ на вопрос
 *   «какой кит выбран сейчас». Это СОСТОЯНИЕ, и в v2 оно становится сервисом плагина
 *   `plugins/kits/registry/`. Сюда из v1 намеренно не переехали `active`/`runtime`/`selection`/`registry`
 *   и React-адаптеры китов: домен не имеет права ни хранить выбор, ни грузить namespace.
 *
 ...
 * них — только типы.
 *
 * @module @reformer/builder-stack-reformer/kits/types
 */
...
export interface KitDescriptor {
   ...
   * (`lib/catalog/make-node`); перенос самих шаблонов в дескриптор — этап 2.
   */
  compoundTemplates?: Record<string, () => JsonNode>;
}

/** Роль записи каталога — реэкспорт, чтобы потребители дескриптора не тянули `lib/catalog/types`. */
