 ...
 * контейнера для drag-раскладки ({@link './node-kind'}), нормализация к flex ({@link './mutate'})
 * и семантическая раскладка агента (в v2 — `plugins/reformer/ai`, импортирует отсюда). Все три раньше
 * сравнивали токен целиком, поэтому адаптивный `md:grid-cols-2` был для них «оформлением»: узел
 * считался вертикальным, а при переключении оси префиксованный грид-класс оставался мусором
 * поверх flex.
 *
 * Модуль — ЛИСТ графа зависимостей: не импортирует ничего.
 *
 * @module @reformer/builder-stack-reformer/form-model/tw-tokens
 */
