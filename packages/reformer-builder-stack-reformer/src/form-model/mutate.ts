 ...
 *
 * @module @reformer/builder-stack-reformer/form-model/mutate
 */
...
import { baseUtility, isAxisToken } from './tw-tokens';
 ...
 *
 * Публичны затем, что обёртку создаёт не только домен: перетаскивание в схематичном виде
 * собирает её из операций редактора (`group` с классом), и класс там обязан быть ТОТ ЖЕ —
 * иначе ряд, собранный мышью, отличался бы от ряда, собранного командой, и «свернуть
 * вырожденную обёртку» узнавало бы только один из них.
 */
export const DEFAULT_ROW_CLASS = 'flex gap-4';
/** Дефолтные классы авто-столбца (вертикальный flex-стек). */
export const DEFAULT_COL_CLASS = 'flex flex-col gap-4';
 ...
 *
 * **Экспортируется, потому что решение «сворачивать ли» принимается НЕ здесь.** Операция правки
 * обязана быть точно обратимой, поэтому сворачивание не прячется внутрь перемещения (там его
 * обратная операция вернула бы узел в контейнер, которого уже нет), а планируется заранее —
 * тем, кто собирает перетаскивание, и отдельной операцией. Планировщику нужен этот же предикат:
 * вторая его копия разошлась бы с первой на первом же `md:grid-cols-2`, и тогда сворачивание
 * происходило бы не там, где его запланировали.
 */
export function isDegenerateWrapper(node: JsonNode | undefined): node is JsonContainerNode {
