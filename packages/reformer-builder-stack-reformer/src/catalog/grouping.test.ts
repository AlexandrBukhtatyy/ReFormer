 ...
 *
 * @module @reformer/builder-stack-reformer/catalog/grouping.test
 */
...
describe('DEFAULT_CATEGORY_ORDER', () => {
  it('перечисляет каждую категорию, которая реально встречается в каталоге', () => {
    const used = [...new Set(builtinEntries().map((e) => e.category ?? FALLBACK_CATEGORY))];
  ...
  it('групповой хвост пуст: незнакомых разделов у дефолтного каталога нет', () => {
    const groups = groupByCategory(builtinEntries()).map(([category]) => category);
