 ...
 *
 * @module @reformer/builder-stack-reformer/catalog/synthetic-entries
 */
...
/**
 * Тоглы синтетических записей — СОБСТВЕННЫЙ входной контракт каталога.
 *
 * В v1 тип назывался `SyntheticConfig` и жил в `config/types` — в контракте runtime-конфига
 * клиента, то есть домен читал форму данных приложения. В v2 конфиг клиента принадлежит хосту, а
 * домен объявляет то, что ему нужно, у себя: секция `components.synthetic` конфига структурно
 * совпадает с этим типом и подставляется вызывающим (плагином) без адаптера.
 */
 ...
 * ({@link SyntheticOptions}) может отключить целые группы или сузить набор HTML-тегов; без конфига —
 * полный набор (поведение как раньше).
 */
export function syntheticRecords(cfg?: SyntheticOptions): CatalogRecord[] {
  const specs =
