...
// (6.0.0 удовлетворяет ">=1.1.0"), а корневой .npmrc глушит остаток флагом
// legacy-peer-deps=true — он нужен для alias-пакетов билдера и убрать его нельзя
// (причина расписана в самом .npmrc). Поэтому здесь всё подменяется руками:
//
