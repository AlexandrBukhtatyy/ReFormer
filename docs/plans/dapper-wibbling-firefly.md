...
- **Линковщик и `PLUGIN_RUNTIME_MODULES` не тронуты (п. 7).** Статическая регистрация втянула бы
  ...
  Сделано 2026-09-19 (ReFormer-es12.6): `PLUGIN_BUNDLED_PACKAGES` в `./tooling` SDK, правило
  в `reformer-plugin build` — пакеты стеков и их внутренности вкладываются, рантайм остаётся внешним.
...
**Сделано 2026‑09‑19.** Пакет `@reformer/builder-stack-plain` (приватный; зависимость одна —
toolkit), плагин `reformer.plain`, профиль `plain.builder`. Живой прогон пройден целиком: в профиле
`plain.builder` схема ReFormer открывается текстом в Monaco, «Файл → Новая простая форма» создаёт
`contact.plain.json`, редактор стека показывает поля и нативную форму поверхностью `plain.native`,
«Добавить поле» правит модель операцией, «Экспорт Form.tsx» пишет файл и сохраняет его в источник
привилегированной службой, дубликат имени подчёркнут в Monaco и назван в панели проблем. Отступления
и добавления:

- **Редактор стека вместо панели превью**: панели превью после Ф2 нет, форму показывает редактор
  документа (слева поля, справа живой вид через `reformer.preview.live`) — тем же приёмом, что
  живой вид редактора схемы.
- **Коды находок с владельцем** (`<plugin-id>:<code>`, `pluginDiagnosticCode`/`splitDiagnosticCode`
  в SDK): второй стек вскрыл, что тексты находок жили только в словаре оболочки. Код с владельцем
  переводит словарь плагина; голые коды — как раньше.
- **Модулей пакета семь**, а не шесть: проверка (`check`) отдельно от разбора — разбор отвечает
  «это схема», проверка — «из неё выйдет форма». Шаблон `Form.tsx` — `.eta` через печатник toolkit.
- **Карта состава** получила запись демо‑стека, поэтому инвариант «полный профиль = вся карта»
  стал двумя: каждая запись востребована хоть одним профилем; полный профиль ReFormer — карта
  без демо‑стека.
- Вскрытые пробелы заведены задачами и закрыты 2026‑09‑19: уведомления с ключом владельца
  (ReFormer-360y), тексты находок платформы (ReFormer-r8oc), отмена модельного документа
  у редактора стека (ReFormer-oa3g).
