...
- **Карта состава** получила запись демо‑стека, поэтому инвариант «полный профиль = вся карта»
  ...
  без демо‑стека.
- Вскрытые пробелы заведены задачами: уведомления переводятся только словарём оболочки
  (ReFormer-360y), нет текста `errors.document.parse-failed` (ReFormer-r8oc), отмена модельного
  документа привязана только редактором схемы (ReFormer-oa3g).
