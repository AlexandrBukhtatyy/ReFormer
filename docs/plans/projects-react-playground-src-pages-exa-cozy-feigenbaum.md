...
const when = (cond) => (...validators) => validators.map(/* условный запуск */);
```
...
- **Фаза 3** ⏳ — i18n + JSON-слот (затрагивает `reformer-cdk`, core types, `reformer-renderer-json`; пересекается с уже идущей правкой meta-схемы по git status).
