...
ReFormer поддерживает адаптеры для популярных библиотек валидации:

### Zod

```typescript
import { zodAdapter } from '@reformer/zod';
...
```typescript
import { yupAdapter } from '@reformer/yup';
...
```typescript
import { valibotAdapter } from '@reformer/valibot';
...
const form = createForm<MyForm>({
  form: { /* ... */ },
  validation: valibotAdapter(schema),
});
```
